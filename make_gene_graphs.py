...
from src.utils.misc import buffer_regs
...
if __name__=="__main__":
    ...
    for chrom in prom_regions:
        for idx in np.arange(0,
                             ...
                             args.chunk_size):
            rdict = {chrom: graph_regions[chrom][idx*args.chunk_size:(idx+1)*args.chunk_size,:]}
            ndict = {chrom: names[chrom][idx*args.chunk_size:(idx+1)*args.chunk_size]}
            tdict = {chrom: target[chrom][idx*args.chunk_size:(idx+1)*args.chunk_size]}
            prom_regs = {chrom: prom_regions[chrom][idx*args.chunk_size:(idx+1)*args.chunk_size,:]}
            ...
            add_binned_data_to_graphlist(glist[chrom],
                                         args.binned_data)
            for idx,item in enumerate(glist[chrom]):
                item['target'] = tdict[chrom][idx]
                item['prom_region'] = prom_regs[chrom][idx,:]
